...
def get_doc_topic_sim(model, extra_doc):
...
def get_top_docs_by_sim(n, model, doc_similarities, reverse=False):
    """ This function returns the top `n` similarities in decreasing order as well
        as the corresponding doc tags.
        Arguments:
            - (int) n: number of top documents to return
            - (gensim.models.doc2vec.Doc2Vec) model: the document embeddings
                model.
            - (np matrix): cosine similarities between a document
                and each doc vector in the model (as returned by `get_cos_sim`
                or `get_doc_topic_sim`)
            - (bool) reverse: if True, will retrieve the "bottom" or "flop" n
                similarities in increasing order
        Returns:
    ...
    # [::-1] reverses the array / [::1] does nothing
    # by default we want to reverse the sort because
    # we want top similarities first
    ...
    top_indices = np.argsort(list(doc_similarities.flat))[::order][:n]
